...
export default function AdminLibrarians() {
  ...
  const filtered = mockLibrarians.filter((l) =>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm" style={{ color: "hsl(220,15%,46%)" }}>Manage library staff</p>
        </div>
        <button className="flex items-center gap-2 rounded-lg px-4 py-2 text-sm font-semibold text-white transition hover:opacity-90"
          ...
          <UserCog className="h-4 w-4" /> + Add Librarian
        </button>
      </div>
      ...
      <div className="rounded-xl border overflow-hidden"
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map((lib) => (
              <tr key={lib.id} style={{ borderTop: "1px solid hsl(220,20%,94%)" }}
                onMouseEnter={(e) => (e.currentTarget.style.background = "hsl(220,20%,98%)")}
                onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
                <td className="px-6 py-4 font-mono text-xs" style={{ color: "hsl(220,15%,55%)" }}>{lib.id}</td>
                <td className="px-6 py-4 font-medium" style={{ color: "hsl(220,60%,15%)" }}>{lib.name}</td>
