    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm" style={{ color: "hsl(220,15%,46%)" }}>Manage book categories</p>
        </div>
      ...
      <div className="rounded-xl border overflow-hidden"
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {filtered.map((cat) => (
              <tr key={cat.id} style={{ borderTop: "1px solid hsl(220,20%,94%)" }}
                onMouseEnter={(e) => (e.currentTarget.style.background = "hsl(220,20%,98%)")}
                onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
                <td className="px-6 py-4 font-mono text-xs" style={{ color: "hsl(220,15%,55%)" }}>{cat.id}</td>
                <td className="px-6 py-4 font-medium" style={{ color: "hsl(220,60%,15%)" }}>{cat.name}</td>
