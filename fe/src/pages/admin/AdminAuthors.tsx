...
const TH: React.CSSProperties = {
  color: "#9c4a70", fontSize: 11, fontWeight: 700,
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#5d2f3a", margin: 0 }}>Authors</h1>
          <p style={{ fontSize: 13, color: "#9e7a7a", marginTop: 4 }}>Manage book authors</p>
        </div>
      ...
      <div style={{ background: "white", borderRadius: 16, overflow: "hidden", border: "1px solid #ffd6e0", boxShadow: "0 2px 16px rgba(244,143,177,0.1)" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          ...
          <tbody>
            {filtered.map((a, i) => (
              <tr key={a.id}
                style={{ borderTop: i === 0 ? "none" : "1px solid #ffe0e8", transition: "background 0.15s" }}
                onMouseEnter={(e) => (e.currentTarget.style.background = "#fff8f8")}
                onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
                <td style={TD}>
                  <span style={{ display: "inline-block", background: "linear-gradient(135deg,#f48fb1,#ffd54f)", color: "white", borderRadius: 6, padding: "2px 8px", fontSize: 11, fontWeight: 600, fontFamily: "monospace" }}>{a.id}</span>
                </td>
                <td style={{ ...TD, fontWeight: 600, fontSize: 14, color: "#5d2f3a" }}>{a.name}</td>
                <td style={TD}>
        ...
        <div style={{ padding: "10px 20px", borderTop: "1px solid #ffe0e8", background: "#fff8f8", fontSize: 12, color: "#b89090" }}>
          Showing {filtered.length} of {mockAuthors.length} authors
        </div>
      </div>
    </div>
  );
