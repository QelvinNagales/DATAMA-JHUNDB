  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#5d2f3a", margin: 0 }}>Members</h1>
          <p style={{ fontSize: 13, color: "#9e7a7a", marginTop: 4 }}>Manage library members</p>
        </div>
        <button style={{ background: "linear-gradient(135deg,#f48fb1,#ffd54f)", color: "white", border: "none", borderRadius: 8, padding: "8px 16px", fontSize: 13, fontWeight: 600, cursor: "pointer" }}>
          + Add Member
        </button>
      </div>
      ...
      <div style={{ background: "white", borderRadius: 16, overflow: "hidden", border: "1px solid #ffd6e0", boxShadow: "0 2px 16px rgba(244,143,177,0.1)" }}>
          ...
          <table style={{ width: "100%", borderCollapse: "collapse" }}>
            <thead>
              <tr style={{ background: "linear-gradient(135deg, #fce4ec, #fff9c4)" }}>
                {["ID", "Name", "Phone", "Email", "Actions"].map((h) => <th key={h} style={TH}>{h}</th>)}
              </tr>
            </thead>
            <tbody>
              {filtered.map((m, i) => (
                <tr key={m.id} style={{ borderTop: i === 0 ? "none" : "1px solid #ffe0e8", transition: "background 0.15s" }}
                  onMouseEnter={(e) => (e.currentTarget.style.background = "#fff8f8")}
                  onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
                  <td style={TD}><span style={{ display: "inline-block", background: "linear-gradient(135deg,#f48fb1,#ffd54f)", color: "white", borderRadius: 6, padding: "2px 8px", fontSize: 11, fontWeight: 600, fontFamily: "monospace" }}>{m.id}</span></td>
                  <td style={TD}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <div style={{ width: 32, height: 32, borderRadius: "50%", background: "linear-gradient(135deg,#f48fb1,#ffd54f)", color: "white", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 700, flexShrink: 0 }}>
                        {m.name.split(" ").map((n) => n[0]).join("").slice(0, 2)}
                      </div>
                      <span style={{ fontWeight: 600, fontSize: 14, color: "#5d2f3a" }}>{m.name}</span>
                    </div>
                  </td>
                  <td style={{ ...TD, fontFamily: "monospace", fontSize: 13, color: "#9e7a7a" }}>{m.phone}</td>
                  <td style={{ ...TD, fontSize: 13, color: "#9e7a7a" }}>{m.email}</td>
                  <td style={TD}>
                    <div style={{ display: "flex", gap: 4 }}>
                      <button title="View" style={{ background: "#fff0f5", border: "none", borderRadius: 6, padding: "6px 8px", cursor: "pointer" }}>
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" style={{ width: 14, height: 14, color: "#f48fb1" }}><path d="M12 4.5C7 4.5 2.73 7.61 1 12c1.73 4.39 6 7.5 11 7.5s9.27-3.11 11-7.5c-1.73-4.39-6-7.5-11-7.5zM12 17c-2.76 0-5-2.24-5-5s2.24-5 5-5 5 2.24 5 5-2.24 5-5 5zm0-8c-1.66 0-3 1.34-3 3s1.34 3 3 3 3-1.34 3-3-1.34-3-3-3z" fill="currentColor"/></svg>
                      </button>
                      <button title="Edit" style={{ background: "#fff0f5", border: "none", borderRadius: 6, padding: "6px 8px", cursor: "pointer" }}>
                      ...
                      <button title="Delete" style={{ background: "#fff0f0", border: "none", borderRadius: 6, padding: "6px 8px", cursor: "pointer" }}>
        ...
        <div style={{ padding: "10px 20px", borderTop: "1px solid #ffe0e8", background: "#fff8f8", fontSize: 12, color: "#b89090" }}>Showing {filtered.length} of {mockMembers.length} members</div>
      </div>
    </div>
  );
}
