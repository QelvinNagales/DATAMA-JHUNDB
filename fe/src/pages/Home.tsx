...
export default function Home() {
  ...
  const resetForm = () => { setEmail(""); setPassword(""); setError(""); setShowPassword(false); setRole("member"); };
  ...
  const handleSearch = (e: React.FormEvent) => { e.preventDefault(); navigate("/member"); };

  return (
    <div className="relative min-h-screen overflow-hidden font-sans text-slate-900" style={{ background: "#fef3c7" }}>

      {/* Warm amber blobs */}
      <div aria-hidden className="pointer-events-none absolute inset-0">
        <div className="absolute -top-32 left-1/2 h-[56rem] w-[56rem] -translate-x-1/2 rounded-full blur-3xl" style={{ background: "rgba(252,211,77,0.55)" }} />
        <div className="absolute -left-40 top-10 h-[48rem] w-[48rem] rounded-full blur-3xl" style={{ background: "rgba(252,165,165,0.45)" }} />
        <div className="absolute -right-40 top-32 h-[52rem] w-[52rem] rounded-full blur-3xl" style={{ background: "rgba(253,186,116,0.4)" }} />
        <div className="absolute -bottom-40 left-1/3 h-[52rem] w-[52rem] -translate-x-1/2 rounded-full blur-3xl" style={{ background: "rgba(251,191,36,0.35)" }} />
      ...
      <div className="relative flex min-h-screen flex-col px-4 sm:px-6 lg:px-8">
        ...
        <header className="flex justify-center pt-6 sm:pt-8">
          <nav className="inline-flex w-full max-w-7xl items-center justify-between rounded-full border border-white/60 bg-white/70 px-4 py-2 shadow-2xl shadow-orange-200/40 backdrop-blur-2xl sm:px-6 sm:py-2.5">
            <span className="text-sm font-semibold tracking-tight text-slate-800 sm:text-base">City Archive</span>
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              {user ? (
                <button type="button" onClick={() => navigate(user.role === "admin" ? "/admin" : "/member")}
        ...
        <main className="flex flex-1 flex-col items-center justify-center pb-28 pt-10 sm:pb-32 sm:pt-12">
          ...
          <form onSubmit={handleSearch}
            className="mt-8 flex w-full max-w-2xl items-center gap-3 rounded-full border border-white/60 bg-white/50 px-4 py-2.5 shadow-2xl shadow-orange-200/40 backdrop-blur-xl sm:px-5 sm:py-3">
            ...
            <button type="submit"
              className="rounded-full bg-slate-900 px-5 py-1.5 text-sm font-semibold text-white shadow-md transition hover:bg-slate-800">
              Search
            </button>
          </form>
        </main>
      ...
      <div className="pointer-events-none fixed inset-x-0 bottom-4 px-4 sm:px-6 lg:px-8">
        <footer className="pointer-events-auto mx-auto flex w-full max-w-7xl items-center justify-between rounded-full border border-white/60 bg-white/70 px-4 py-2 text-xs text-slate-500 shadow-2xl shadow-orange-200/40 backdrop-blur-2xl sm:px-6">
      ...
      {modal === "login" && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <div className="w-full max-w-md overflow-hidden rounded-3xl border border-white/60 bg-white/85 shadow-2xl backdrop-blur-2xl"
            ...
            <div className="px-6 py-5">
              ...
              <div className="grid grid-cols-2 gap-2 mb-5">
                {(["member", "admin"] as const).map((r) => (
                  <button key={r} type="button" onClick={() => { setRole(r); setError(""); }}
                    ...
                    style={{
                      borderColor: role === r ? "hsl(30,95%,50%)" : "hsl(30,20%,88%)",
                      background: role === r ? "hsl(30,95%,50%)" : "rgba(255,255,255,0.6)",
              ...
              {/* Hint */}
              <div className="mb-4 rounded-xl px-4 py-2.5 text-xs text-slate-500" style={{ background: "rgba(255,251,235,0.9)" }}>
                {role === "admin"
              ...
              <form onSubmit={handleLogin} className="space-y-4">
                <div className="space-y-1.5">
                  ...
                  <input type="email" required placeholder="you@example.com"
                    ...
                    onFocus={(e) => e.target.style.borderColor = "hsl(30,95%,50%)"}
                    onBlur={(e) => e.target.style.borderColor = "hsl(30,20%,88%)"}
                  />
                </div>
                <div className="space-y-1.5">
              ...
              <p className="mt-4 text-center text-xs text-slate-500">
                ...
                <a href="/register" className="font-semibold underline underline-offset-2" style={{ color: "hsl(30,95%,45%)" }}>
                  Register here
                </a>
              </p>
            </div>
