...
const mockCategories = [
  { id: "C001", name: "Fiction" },
  { id: "C002", name: "Science Fiction" },
  { id: "C003", name: "Romance" },
  { id: "C004", name: "History" },
  { id: "C005", name: "Biography" },
  { id: "C006", name: "Adventure" },
];
...
export default function AdminCategories() {
  ...
  const filtered = mockCategories.filter((c) => c.name.toLowerCase().includes(search.toLowerCase()));

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between" }}>
        <div>
          <h1 style={{ fontSize: 22, fontWeight: 700, color: "#5d2f3a", margin: 0 }}>Categories</h1>
          <p style={{ fontSize: 13, color: "#9e7a7a", marginTop: 4 }}>Manage book categories</p>
        </div>
      ...
      <div style={{ background: "white", borderRadius: 16, overflow: "hidden", border: "1px solid #ffd6e0", boxShadow: "0 2px 16px rgba(244,143,177,0.1)" }}>
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ background: "linear-gradient(135deg, #fce4ec, #fff9c4)" }}>
              {["ID", "Category", "Actions"].map((h) => <th key={h} style={TH}>{h}</th>)}
            </tr>
          </thead>
          <tbody>
            {filtered.map((c, i) => (
              <tr key={c.id} style={{ borderTop: i === 0 ? "none" : "1px solid #ffe0e8", transition: "background 0.15s" }}
                onMouseEnter={(e) => (e.currentTarget.style.background = "#fff8f8")}
                onMouseLeave={(e) => (e.currentTarget.style.background = "transparent")}>
                <td style={TD}><span style={{ display: "inline-block", background: "linear-gradient(135deg,#f48fb1,#ffd54f)", color: "white", borderRadius: 6, padding: "2px 8px", fontSize: 11, fontWeight: 600, fontFamily: "monospace" }}>{c.id}</span></td>
                <td style={{ ...TD, fontWeight: 600, fontSize: 14, color: "#5d2f3a" }}>{c.name}</td>
                <td style={TD}>
        ...
        <div style={{ padding: "10px 20px", borderTop: "1px solid #ffe0e8", background: "#fff8f8", fontSize: 12, color: "#b89090" }}>Showing {filtered.length} of {mockCategories.length} categories</div>
      </div>
    </div>
  );
}
